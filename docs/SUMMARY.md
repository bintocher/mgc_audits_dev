...
2. **PROJECT_DESIGN.md** - детальное описание архитектуры и модели данных
3. **PROJECT_PLAN.md** - план разработки, API, оценка трудозатрат
4. **questions.md** - вопросы для заказчика

Файлы 2 и 3 объеденены в 1.
...
- Создание планов аудитов на год/квартал
...
- **Dictionary** (Process, Norm, AuditType, Product, Project) - справочники
- **Status, StatusTransition** - workflow с поддержкой цветов
- **AuditPlan, AuditPlanItem** - планы аудитов
- **AuditorQualification** - квалификация аудиторов
- **Audit** - аудиты (с поддержкой переносов, часов, результатов)
- **AuditComponent** - компоненты (детали, узлы) в рамках аудита (новое)
...
REST API с JWT авторизацией, `/api/v1`, около 70+ эндпоинтов.
...
Детальная спецификация в файле `PROJECT_PLAN.md`.
...
- Создание и редактирование аудитов
...
- Исправлены все замечания после MVP
- Система стабильна под нагрузкой
- История изменений работает
- Универсальные эндпоинты истории изменений доступны
...
1. **Получить ответы на вопросы** из файла `questions.md`
2. **Скорректировать оценку** с учетом ответов
3. **Предоставить доступы:** сервер, SMTP, LDAP, Telegram бот
...
3. `questions.md` - вопросы для заказчика
